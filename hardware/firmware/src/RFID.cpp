...
void RFID_init() {
  ...
  if (version == 0x00 || version == 0xFF) {
    SensorHealthManager::updateSensorHealth("mfrc522", SENSOR_DISCONNECTED, nullptr, "MFRC522 SPI failed");
    Serial.println("[RFID] MFRC522 SPI communication failed");
    return;
  }
  ...
  if ((antennaGain & 0x70) == 0x70) {
    SensorHealthManager::updateSensorHealth("mfrc522", SENSOR_OK, "MFRC522 initialized");
  } else {
    SensorHealthManager::updateSensorHealth("mfrc522", SENSOR_INIT_FAILED, nullptr, "Antenna init failed");
  }
  ...
  // SD card initialization is handled by SDCardManager
  // No need to initialize here as it's done in main setup()
  Serial.println("✅ SD card will be used for room data persistence");
...
void RFID_update(SensorData* data) {
  ...
  if (sensorsDisabled) {
    ...
    // No card present - check if sensor is still responsive
    byte testReg = mfrc522.PCD_ReadRegister(MFRC522::VersionReg);
    if (testReg == 0x00 || testReg == 0xFF) {
      SensorHealthManager::updateSensorHealth("mfrc522", SENSOR_ERROR, nullptr, "MFRC522 unresponsive");
    }
  }
  ...
  if (currentTagState && !tagPresent) {
    if (mfrc522.PICC_ReadCardSerial()) {
      ...
      // Update health status on successful read
      SensorHealthManager::updateSensorHealth("mfrc522", SENSOR_OK, "Card read OK");
...
void RFID_saveRoomData() {
  if (!SDCard_writeFile("/data/rooms.dat", (uint8_t*)roomDataArray, sizeof(roomDataArray))) {
    Serial.println("❌ Failed to save room data to SD card");
  } else {
    Serial.println("✅ Room data saved to SD card");
  }
}

void RFID_loadRoomData() {
  size_t bytesRead;
  if (SDCard_readFile("/data/rooms.dat", (uint8_t*)roomDataArray, sizeof(roomDataArray), &bytesRead)) {
    if (bytesRead == sizeof(roomDataArray)) {
      for (int i = 0; i < MAX_ROOMS; i++) {
      ...
      Serial.println("✅ Room data loaded from SD card");
    } else {
      Serial.println("⚠️ Room data file size mismatch");
    }
  } else {
    Serial.println("ℹ️ No saved room data found on SD card");
  }
}
