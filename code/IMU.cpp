...
enum FallState { FALL_NONE, FALL_DETECTED, FALL_CONFIRMED };
...
static void factoryResetEEPROM() {
  ...
  // Also clear step data
  StepData stepData;
  stepData.magic = 0;
  EEPROM.put(STEP_DATA_ADDR, stepData);
  
  EEPROM.commit();
  Serial.println("EEPROM Factory Reset - Calibration and step data cleared");
}
...
static void saveStepDataToEEPROM() {
  ...
  EEPROM.put(STEP_DATA_ADDR, data);
  EEPROM.commit();
#ifdef SC_DEBUG_IMU
...
static bool loadStepDataFromEEPROM() {
  StepData data;
  EEPROM.get(STEP_DATA_ADDR, data);
  
  if (data.magic != STEP_DATA_MAGIC) {
#ifdef SC_DEBUG_IMU
    Serial.println("No valid step data found in EEPROM");
#endif
    return false;
  }
...
static void configureMPU() {
  ...
  Wire.beginTransmission(MPU_ADDR); Wire.write(0x19); Wire.write(0x04); Wire.endTransmission();
  Wire.beginTransmission(MPU_ADDR); Wire.write(0x1C); Wire.write(0x10); Wire.endTransmission();
  Wire.beginTransmission(MPU_ADDR); Wire.write(0x1B); Wire.write(0x10); Wire.endTransmission();
  i2cErrorCount = 0;
}
...
void IMU_init() {
  ...
  EEPROM.begin(sizeof(CalibrationData) + sizeof(StepData));
