...
static void calibrateSensor() {
  ...
  if (validCal && stable) {
    ...
    Serial.println(" lux");
    preferences.putFloat("darkOffset", darkOffset);
...
void checkSerialForCalibrationReset() {
  if (Serial.available()) {
    ...
    if (cmd == "resetcal") {
      ...
      preferences.remove("darkOffset");
      preferences.remove("calibrated");
      preferences.end();
...
void LightSensor_init() {
  ...
  preferences.begin("calibration", false);
  if (preferences.isKey("darkOffset")) {
    darkOffset = preferences.getFloat("darkOffset", 5.0);
    calibratedDark = preferences.getBool("calibrated", false);
